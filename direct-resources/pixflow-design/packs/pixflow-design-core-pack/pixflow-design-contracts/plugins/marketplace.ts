...
import {
  LocalizedTextSchema,
  PIXFLOW_DESIGN_PLUGIN_SPEC_VERSION,
  PixflowDesignSpecVersionSchema,
} from './manifest.js';
...
// `pixflow-design-marketplace.json` schema (v1). Mirrors
// `docs/schemas/pixflow-design.marketplace.v1.json`. The federated catalog
...
export const MarketplaceManifestSchema = z.object({
  ...
  specVersion: PixflowDesignSpecVersionSchema.default(PIXFLOW_DESIGN_PLUGIN_SPEC_VERSION),
  name:        z.string().min(1),
  version:     z.string().min(1),
  owner: z.object({
