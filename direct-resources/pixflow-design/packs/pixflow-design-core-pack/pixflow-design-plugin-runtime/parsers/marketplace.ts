import { MarketplaceManifestSchema, type MarketplaceManifest } from '@pixflow-design/contracts';
...
export function parseMarketplace(raw: string): MarketplaceParseResult {
  ...
  try {
    ...
    return { ok: false, errors: [`pixflow-design-marketplace.json is not valid JSON: ${(err as Error).message}`] };
  }
