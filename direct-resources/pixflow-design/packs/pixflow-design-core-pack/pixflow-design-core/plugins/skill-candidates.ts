...
import { PIXFLOW_DESIGN_PLUGIN_SPEC_VERSION } from '@pixflow-design/contracts';
...
export async function generateSkillPluginDraft(
  ...
  await fs.writeFile(path.join(folder, 'pixflow-design.json'), JSON.stringify(buildManifest(slug, candidate), null, 2) + '\n', 'utf8');
...
function isExplicitSkillPluginUrl(value: string): boolean {
  ...
  return /\/(?:SKILL\.md|pixflow-design\.json)$/u.test(decodeURIComponent(url.pathname));
}
...
function synthesizeSkill(candidate: SkillPluginCandidate): string {
  ...
  if (source) return `${source}\n\n## Provenance\n\nFormalized by Pixflow Design from candidate ${candidate.id}.\n`;
  return [
    `# ${candidate.title}`,
    '',
    candidate.description,
    ...
    '## When to use',
    '',
    'Use this skill when the workflow described by the source material should be repeated inside Pixflow Design.',
    '',
    '## Workflow',
    '',
...
function buildManifest(slug: string, candidate: SkillPluginCandidate) {
  return {
    $schema: 'https://pixflow-design.ai/schemas/plugin.v1.json',
    specVersion: PIXFLOW_DESIGN_PLUGIN_SPEC_VERSION,
    name: slug,
