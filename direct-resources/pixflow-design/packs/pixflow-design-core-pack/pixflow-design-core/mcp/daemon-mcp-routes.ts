...
import { SIDECAR_ENV } from '@pixflow-design/sidecar-proto';
...
export function registerMcpRoutes(app: Express, ctx: RegisterMcpRoutesDeps) {
  ...
  function computeInstallPayload(): McpInstallPayload {
    ...
    // knows where the browser-facing Pixflow Design studio is running.
  ...
  const CODEX_MCP_NAME = 'pixflow-design';
  ...
  // External MCP server configuration. Pixflow Design connects to these as a
...
function renderOAuthResultPage(opts: any) {
  ...
  const body = ok
    ? `Your MCP server <code>${escapeHtml(opts.serverId ?? '')}</code> is now connected. You can close this tab and return to Pixflow Design.`
...
<head>
<meta charset="utf-8" />
<title>${escapeHtml(title)} — Pixflow Design</title>
  ...
  <script>
    try {
      ...
      if (window.BroadcastChannel) {
        var bc = new BroadcastChannel('pixflow-design-mcp-oauth');
