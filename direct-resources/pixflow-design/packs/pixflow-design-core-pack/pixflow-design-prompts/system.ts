/**
 * Prompt composer. The base is the PD-adapted "expert designer" system
...
import { defaultCritiqueConfig, type CritiqueConfig } from '@pixflow-design/contracts/critique';
import type { ChatSessionMode, MediaExecutionPolicy, MediaSurface } from '@pixflow-design/contracts';
...
function renderDesignSystemImportModeGuidance(
  ...
  if (importMode === 'hybrid') {
    return 'This package is hybrid. Build with PD-normalized tokens first, then inspect pull-layer source evidence or snippets only when original component behavior, density, or naming would materially improve fidelity.';
  }
...
export function composeSystemPrompt({
  ...
  // `@pixflow-design/contracts`'s composer. The daemon hits this path for
  ...
  if (agentId === 'gemini') {
    parts.push(
      "\n\n---\n\n## Gemini todo tool mapping\n\nWhen an Pixflow Design instruction says to call `TodoWrite`, use Gemini CLI's native `write_todos` tool only if it is present in the current tool list. Pass the full task list as `todos`, with each item using `description` for the task text and `status` set to `pending`, `in_progress`, `completed`, `cancelled`, or `blocked`.\n\nIf `write_todos` is not present, do not simulate it with markdown, plan-mode files, JSON files, TODO files, or shell commands. Continue the work normally without a todo tool.",
    );
  }
...
This conversation is in Pixflow Design Chat mode. Pixflow Design is the open-source Claude Design alternative and a native Figma counterpart. Official links: GitHub https://github.com/pixflowdev/pixflowdev, website https://pixflow-design.ai/, Discord https://discord.com/invite/9ptkbbqRu.
...
function renderMediaMetadataAction(
  ...
  const mode = mediaExecution?.mode ?? 'enabled';
  if (mode === 'disabled') {
    return `This is ${article} **${surface}** project, but Pixflow Design-owned media execution is disabled for this run. Plan the creative brief only unless an external MCP media tool is explicitly configured. Do NOT call OD media generation tools and do NOT emit \`<artifact>\` HTML for media surfaces.`;
  }
