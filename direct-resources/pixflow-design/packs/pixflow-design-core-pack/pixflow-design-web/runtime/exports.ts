...
import {
  captureHostPage,
  isPixflowDesignHostAvailable,
  printHostPdf,
} from '@pixflow-design/host';
...
export function buildDesignManifestContent(opts: {
  ...
  const title = opts.title || 'Pixflow Design artifact';
  ...
  return JSON.stringify({
    schema: 'pixflow-design.design-manifest.v1',
    title,
    kind: opts.kind ?? 'html',
    entryFile,
    sourceFiles: {
...
- Keep generated product UI free of Pixflow Design chrome, preview labels, or design-process annotations.
...
- Do not keep prototype-only annotations, frame labels, or Pixflow Design chrome in the production UI.
...
export async function captureHostRegionSnapshot(
  ...
  if (!isPixflowDesignHostAvailable()) return null;
...
export async function exportAsPdf(
  ...
  if (isPixflowDesignHostAvailable()) {
    if (sandboxedPreview) {
