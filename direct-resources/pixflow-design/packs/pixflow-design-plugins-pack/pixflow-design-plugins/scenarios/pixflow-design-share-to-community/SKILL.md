---
name: pixflow-design-share-to-community
description: Package the user's just-finished work as an Pixflow Design plugin without asking for fields the project files already answer, then surface the existing Add-to-My-plugins / Pixflow-Design-PR buttons.
od:
  scenario: plugin-sharing
  mode: scenario
---

# pixflow-design-share-to-community (scenario)

Triggered by the post-completion "Share to Pixflow Design" submission action. The user just finished a piece of work in this project and wants to ship it as a plugin. They have not been asked any questions yet.
...
## Do NOT chain the publish-repo / Pixflow-Design-PR flows yourself

Do NOT suggest follow-up CLI commands such as `od plugin publish`, `od plugin publish --to pixflow-design`, `gh repo create`, `git init` / `git remote add` / `git push`, or any other publish / repo wiring. The plugin-folder card under Design Files already exposes three buttons whose prompts drive those flows end-to-end with the right auth gates, fallbacks, and retry rules baked in:
...
- **Pixflow Design PR** — opens a draft PR against `pixflowdev/pixflowdev` for the community catalog.
...
- `docs/schemas/pixflow-design.plugin.v1.json`
- The sibling `plugins/_official/scenarios/pixflow-design-plugin-authoring/SKILL.md` for the from-scratch authoring counterpart.
