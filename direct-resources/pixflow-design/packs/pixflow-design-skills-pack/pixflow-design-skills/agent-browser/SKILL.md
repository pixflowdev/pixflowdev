...
description: |
  ...
  Pixflow Design browser-tab context, testing web apps, dogfooding Pixflow Design
  previews, QA, bug hunts, or reviewing app quality. Prefer local Pixflow Design
...
Use `agent-browser` for local Pixflow Design preview validation: inspect rendered
...
For selected Pixflow Design browser tabs and browser-use/browser-harness-style
...
5. If the selected Pixflow Design context only provided a URL/title and no browser
...
Do not run Pixflow Design's own daemon CLI as a browser automation tool. Commands
...
```bash
CHROME_USER_DATA_DIR=/tmp/pixflow-design-agent-browser-chrome
