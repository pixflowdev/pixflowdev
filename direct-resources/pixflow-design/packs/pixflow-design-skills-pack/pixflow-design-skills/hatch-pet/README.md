...
checked into the Pixflow Design repo (rather than pulled in as a Git submodule
or an npm package) so that:

- Any Pixflow Design agent can run the skill end-to-end without a network
...
- Pinned upstream reference (declared in `SKILL.md` frontmatter): see the
  ...
  directory as the authoritative source-of-truth for Pixflow Design and
...
2. Replace the contents of this directory with the upstream snapshot,
   preserving only this `README.md` and any Pixflow-Design-specific notes
   inside `SKILL.md`'s `> **Pixflow Design integration.**` blockquote.
...
flow (including how Pixflow Design behaves when Codex is not installed).
